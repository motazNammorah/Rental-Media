...
public class UpdateMove extends Application implements EventHandler<ActionEvent>{
	...
	public void test(Button bt2,Button bt3) {
		bt2.setOnAction(e->{
			...
			while(i<r.media.size()) {
				if(((Movie)r.media.get(i)).getTheCode().equals(te.getText())) {
					o=i;
					break;
				}
				i++;
			...
			else
			{
				...
				String s1=((Movie)r.media.get(o)).title;
				String s2=((Movie)r.media.get(o)).getCopiesAvailable()+"";
				String s3=((Movie)r.media.get(o)).getRating();
				te1.setText(s1);
				te2.setText(s2);
				te3.setText(s3);
				...
				bt3.setOnAction(e1->{
					...
					while(j<r.media.size()) {
						if(((Movie)r.media.get(j)).getTheCode().compareTo(te.getText())==0) {
							v=j;
							break;
						}
						j++;
				...
				((Movie)r.media.get(v)).setTitel(title);
				((Movie)r.media.get(v)).setCopiesAvailable(h);
				((Movie)r.media.get(v)).setRating(te3.getText());
				});
			}
